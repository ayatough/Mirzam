...
/// A stable name for what a build warning is about, and where in the source
/// it is: both now have three readers — the JSON report, the language server
/// and the browser editor — so both live in `mirzam-render::diagnose`, and
/// this is where `check` has always reached for the first of them.
pub use mirzam_render::diagnose::warning_kind;
