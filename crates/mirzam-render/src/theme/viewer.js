(() => {
  ...
  // The connector layer, likewise absent from decks that connect nothing. It
  // lives in `connect.js` rather than here because the print, handout and shot
  // pages run it too: a route is resolved from the laid-out page, so an export
  // that never runs the viewer would otherwise print a slide with no arrows.
  const connect = () => window.MZConnect || null;
  ...
  function show(i, opts) {
    ...
    renderNotes();
    // Connectors resolve their endpoints once layout has settled. The routing
    // is `connect.js`'s, so a deck that connects nothing carries none of it.
    requestAnimationFrame(() => { const c = connect(); if (c) c.draw(sec); });
  }
