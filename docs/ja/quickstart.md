...
- **確認:** ビルド済み `.html` をファイルアプリや共有から開くだけ。スワイプでページ
  送り、上スワイプでノート、ピンチで拡大、2 本指タップでショートカット一覧。
- **発表:** デッキは 1 ファイルなので、AirDrop かクラウドフォルダに置けば配布は完了です。

### 読みやすく表示する

スライドは横長で、縦に持ったスマホはそうではありません。タブで開いたデッキが画面
中央の細い帯になり、上にアドレスバー、下にツールバーが残るのはそのためです。効果の
大きい順に 3 つ:

- **コントロールの ⛶ をタップ。** デッキが画面いっぱいに広がり、机の上で見るときの
  余白と影を外し、さらに（横長スライド × 縦画面という問題そのものに対して）画面を
  横向きに回すよう要求します。キーボードなら `F`。ボタンはフルスクリーンに対応した
  ブラウザにだけ出ます。iPhone の Safari は対応していないので、その場合は次の 2 つを。
- **スマホを横にする。** 何も押さずに済み、効果はこれが大半です。スライドと画面の
  縦横比がようやく一致し、横向きではブラウザ自身のバーも細くなります。
...
上に縦一列で開きます。横向きにするか、これより広い画面では全部が 1 行に並び、**⋯**
自体が現れません。コントロールがスライドに重なることはありません——タッチ画面では
このボタン群だけが唯一の操作手段なので常時表示のままにし、代わりにデッキ側が
（幅と高さのうち、スライドが小さくならない方を譲って）その手前で止まります。

それでも読めない細部——軸ラベル、出典、図のキャプション——は **ピンチ**で拡大でき
ます。画面にデッキを収めているのと同じ transform を掛け直すだけなので、文字は
ぼやけず描き直され、ドラッグで拡大したまま移動できます。全画面でも効くのが要点で、
ブラウザ自身のズームは全画面では働かない——つまりスマホでスライドを読む唯一の
モードで拡大できない——ためにデッキ側で拡大しています。拡大中はスワイプとタップ
によるページ送りが止まり（勝手にスライドが変わらない）、**‹** **›** ボタンは
そのまま使えて、ページを送ると全体表示に戻ります。

ショートカット一覧（2 本指タップ、または **?** ボタン）には、いま手にしている
ブラウザで使える方法だけが並びます。
