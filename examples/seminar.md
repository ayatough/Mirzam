...
1. **背景**: 分散読み出しとその限界
2. **課題**: 前回までの測定で見えたこと
3. **手法**: JPA 導入と積分時間の最適化
4. **結果**: 忠実度 {{f_base}}% → **{{f_jpa}}%**
5. **考察と今後**
...
::: pane main
- 量子ビットと共振器を**分散結合**させると、量子ビットの状態に応じて共振周波数が $\chi = g^2 / \Delta$ だけシフトする
...
*参考: 先行研究では JPA 導入により SNR が 5–10 倍改善*
:::
...
::: pane main
- 初段に **JPA**(ジョセフソン・パラメトリック増幅器)を挿入し、量子限界近くの雑音温度を狙う
- 積分時間 $\tau$ は長いほど SNR が稼げるが、$T_1$ 緩和による**状態反転**と競合する
...
- 次の一手は **TWPA**(進行波型)への置き換え: 飽和点が高く、帯域も広い
