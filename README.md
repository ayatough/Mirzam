...
binds. `V` shows the Markdown a slide was written as, beside the slide —
always under `mirzam serve`, and in a built deck with `--embed-source`. Add
`--editor-url` and one click opens the whole deck in the browser editor at
that slide, which is how every deck on
